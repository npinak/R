...
#You need to create three plots that potray the following insights:
# 1) Free throw attempts per game
# 2) Accuracy of Free Throws
# 3) Player playing style (2 vs 3 point preference) excluding free throws


# Create a function to plot quickly ----
myplot <- function(data, rows = 1:10){
  Data <- data[rows,,drop=F]
  matplot(t(Data), type="b", pch=15:18, col=c(1:4,6))
  #legend("bottomleft", inset=0.01, cex = 0.55, legend=Players[rows],col=c(1:4,6),pch=15:18, horiz=F)
  legend("bottomleft",  inset = c(0,0) , cex = 0.55, col = c(1:4,6),pch=15:19, 
         lty = 1, lwd = 2, ncol = 5, legend = Players)
}
...
# calculate ratio of points:fieldgoals. Have to subtract free throws first

Points.No.FreeThrows <- Points - FreeThrows 


myplot(Points.No.FreeThrows/FieldGoals)
